...
void Enemy::updateVelocity() {
    ...
    first.y = game->player->x;
    SDL_FPoint second;
    second.x = enemy_image->x + (enemy_image->height / 2 * enemy_image->scale);
    second.y = enemy_image->y + (enemy_image->width / 2 * enemy_image->scale);
    game->utils->calc(&first, &second, &velocity);

    enemy_image->x += velocity.x * 10;
    enemy_image->y += velocity.y * 10;
}
